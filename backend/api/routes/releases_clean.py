...
def create_release(payload: ReleaseCreate, db: Session = Depends(get_db)):
    ...
    logger.info("app.backend", f"Legacy release creation - redirecting to new system", "legacy_release_redirect", {
    ...
    try:
        ...
        from core.release_controller import ReleaseConfig
        ...
        release_id = controller.generate_release(
            ...
            description=payload.description,
            config=config,
            transformations=payload.transformations or []
        )
        
        logger.info("operations.releases", f"Release created successfully using new system", "legacy_release_success", {
            "release_id": release_id,
            "project_id": project_id
        })
        ...
        logger.error("errors.system", f"Legacy release creation failed: {str(e)}", "legacy_release_error", {
            "error": str(e)
        })
        raise HTTPException(status_code=500, detail=str(e))
...
def get_release_history(dataset_id: str, db: Session = Depends(get_db)):
    ...
    try:
        # Query releases that contain this dataset
        releases = db.query(Release).filter(
            Release.datasets_used.contains([dataset_id])
        ).order_by(Release.created_at.desc()).all()
        
        logger.info("operations.releases", f"Found {len(releases)} releases for dataset", "release_history_found", {
            "dataset_id": dataset_id,
        ...
        for release in releases:
            release_history.append({
                ...
                "train_image_count": release.train_image_count,
                "val_image_count": release.val_image_count,
                "test_image_count": release.test_image_count,
        ...
        for split, images in all_images_by_split.items():
            ...
            for img_data in images:
                ...
                if os.path.exists(original_path):
                    ...
                    # Create full transformation config for annotation transformation
                    full_transform_config = None
                    try:
                        if transformations:
                    ...
                    label_content = None
                    try:
                        if full_transform_config:
                            # Use the existing annotation transformation system
                            ...
                            # Convert database annotations to annotation objects
                            annotation_objects = []
                            for ann in img_data["annotations"]:
                                try:
                                    if hasattr(ann, 'polygon_points') and ann.polygon_points:
                                        # Polygon annotation
                                        points = [(p.get('x', 0), p.get('y', 0)) for p in ann.polygon_points]
                                        polygon = Polygon(
                                            points=points,
                                            class_name=getattr(ann, 'class_name', f"class_{getattr(ann, 'class_id', 0)}"),
                                            class_id=getattr(ann, 'class_id', 0)
                                        )
                                        annotation_objects.append(polygon)
                                    else:
                                        # Bounding box annotation
                                        x = getattr(ann, 'x', 0)
                                        y = getattr(ann, 'y', 0)
                                        width = getattr(ann, 'width', 0)
                                        height = getattr(ann, 'height', 0)
                                        bbox = BoundingBox(
                                            x_min=x,
                                            y_min=y,
                                            x_max=x + width,
                                        ...
                                        annotation_objects.append(bbox)
                                except Exception:
                                    continue
                            
                            # Transform annotations using the centralized system
                            original_dims = (int(getattr(img_data["db_image"], 'width', 640)), 
                                           int(getattr(img_data["db_image"], 'height', 480)))
                            new_dims = (img_w, img_h)
                            
                            transformed_annotations = update_annotations_for_transformations(
                                annotation_objects, full_transform_config, original_dims, new_dims
                            )
                            
                            # Convert transformed annotations to YOLO format
                            yolo_lines = []
                            for ann in transformed_annotations:
                                try:
                                    class_idx = resolve_class_index(ann) if resolve_class_index else ann.class_id
                                    
                                    if isinstance(ann, BoundingBox):
                                        # YOLO detection format
                                        if label_mode == "yolo_detection":
                                            center_x = (ann.x_min + ann.x_max) / 2 / img_w
                                            center_y = (ann.y_min + ann.y_max) / 2 / img_h
                                            norm_width = (ann.x_max - ann.x_min) / img_w
                                            norm_height = (ann.y_max - ann.y_min) / img_h
                                            ...
                                            yolo_lines.append(f"{class_idx} {center_x:.6f} {center_y:.6f} {norm_width:.6f} {norm_height:.6f}")
                                        ...
                                        if label_mode == "yolo_segmentation":
                                            ...
                                            for x, y in ann.points:
                                                norm_x = max(0, min(1, x / img_w))
                                                norm_y = max(0, min(1, y / img_h))
                            ...
                            # No transformations: use original content
                    ...
                    for aug_idx, plan in enumerate(aug_plan, start=1):
                        ...
                        if augmented_image:
                                ...
                                try:
                                    ...
                                    transformed_annotations = update_annotations_for_transformations(
                                        annotation_objects, config_dict, original_dims, new_dims
                                    )
                                    ...
                                    # Fallback: try to use transformation functions, otherwise use original
                                    try:
                                        if label_mode == "yolo_detection":
                                        ...
                                        logger.warning("errors.system", f"Fallback aug label creation also failed", "aug_labels_fallback_error", {
                                            'aug_filename': aug_filename,
                                            'error': str(_e2)
                                        })
                                        with open(aug_label_path, 'w') as f:
